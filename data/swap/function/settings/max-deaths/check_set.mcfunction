...
# Check if it's valid
execute if score _input_value _settings matches ..0 run return run function swap:settings/max-deaths/direct_set
...
execute if score _input_value _settings matches 201..10000 run return run tellraw @a[tag=op] [{"color":"gold","text":"Are you sure you would like to set the worldborder value to "},{"color":"light_purple","score":{"name":"_input_value","objective":"_settings"}},{"color":"gold","text":"?\n\n"},{"clickEvent":{"action":"run_command","value":"/function swap:settings/max-deaths/set"},"color":"green","text":"[Yes]"},"     ",{"clickEvent":{"action":"run_command","value":"/function swap:settings/show_settings"},"color":"red","text":"[No]"}]
